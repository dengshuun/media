...
public class PhotoView extends ImageView {
	...
	private void init() {
		...
		setMAX_OVER_SCROLL((int) (density * 30));
	...
	public boolean dispatchTouchEvent(MotionEvent event) {
		if (isEnable) {
			...
			if (event.getPointerCount() >= 2)
				hasMultiTouch = true;
			if (Action == MotionEvent.ACTION_DOWN && mOnImageTouchListener != null)
				mOnImageTouchListener.ImageTouchDown();
	...
	private class Transform implements Runnable {
		...
		void withFling(float velocityX, float velocityY) {
			...
			mFlingScroller.fling(mLastFlingX, mLastFlingY, (int) velocityX, (int) velocityY, minX, maxX, minY, maxY,
					Math.abs(overX) < MAX_FLING_OVER_SCROLL * 2 ? 0 : MAX_FLING_OVER_SCROLL, Math.abs(overY) < MAX_FLING_OVER_SCROLL * 2 ? 0
	...
	public int getMAX_OVER_SCROLL() {
		return MAX_OVER_SCROLL;
	}

	public void setMAX_OVER_SCROLL(int mAX_OVER_SCROLL) {
		MAX_OVER_SCROLL = mAX_OVER_SCROLL;
	}
}