...
public class UsbPictureActivity extends Activity implements OnClickListener {
	...
	protected void onResume() {
		...
		bindService(new Intent("com.hwatong.media.MediaScannerService"), mMediaServiceConnection, 0);
	...
	private final ServiceConnection mMediaServiceConnection = new ServiceConnection() {
		...
		public void onServiceConnected(ComponentName name, IBinder service) {
			...
			try {
				String state = mService.getUsbPrescanState(Constant.ROOT_DIR_PATH);

				if ("prescan".equals(state)) {
					if (Constant.DEBUG)
						Log.i(Constant.TAG_USB_PICTURE, "prescan in progress");
					mLoadingDialog.show();
				} else {
					mLoadingDialog.dismiss();
					if (!mService.getVolumeState(Constant.ROOT_DIR_PATH)) {
						finish();
					}
				}
			} catch (RemoteException e) {
	...
	public void onClick(View v) {
		...
		case R.id.btn_picture_resize:
			if (isFullScreen) {
				quitFullScreen();
				if (mPictureControlView.getVisibility() == View.GONE)
	...
	private static final int MSG_TIME_PLAY = 0x1011;// 播放图片时间更新
